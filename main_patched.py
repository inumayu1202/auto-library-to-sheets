...
def main():
    ...
    for attempt in range(1, max_retries + 1):
        if attempt == 1:
            ...
            print(f"-> 2. Google SpreadSheet への接続を再試行します (試行 {attempt}/{max_retries})")
        try:
            scopes = [
            ...
            # 背景色（ハイライト）をクリアするリクエスト（A1:Z1000の範囲）
            clear_bg_request = {
                "requests": [
                    {
                        "repeatCell": {
            ...
            if not res_list:
                ...
                worksheet.clear()
                try:
                    sh.batch_update(clear_bg_request)
                except gspread.exceptions.APIError:
                    raise
                except Exception as e:
                    print(f"背景色のクリア中にエラーが発生しました: {e}")
            ...
            write_list = [header]
            for row in res_list:
                date_text, title, author, publisher, year = row[:5]
                write_list.append([date_text, title, author, publisher, year])
                
            # 後の処理のためにheaderを挿入
            res_list.insert(0, header)
            
            # A1セルから一括で書き込み
            worksheet.append_rows(write_list, value_input_option='USER_ENTERED')
